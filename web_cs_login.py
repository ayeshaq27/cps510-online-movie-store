...
# Blueprint for Customer Service login routes
cs_bp = Blueprint('cs', __name__, template_folder='templates')


@cs_bp.route('/', methods=['GET'])
def index():
    # Renders the CS login form. When this blueprint is registered under a URL prefix
    # the front-page for the blueprint will be mounted at that prefix (e.g. /cs/).
...
@cs_bp.route('/api/login', methods=['POST'])
def api_login():
